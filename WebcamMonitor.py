...
DEFAULT_CONFIG = {
    "VIDEO": {
        ...
        "MAX_DURATION": "1800",  # 秒
        "MAX_SIZE": "104857600" # 100MBをバイトで表現
    }
}
...
def load_config():
    ...
    if not config["VIDEO"]["SOURCE"].isdigit() or int(config["VIDEO"]["SOURCE"]) < 0: # ソースが数字でないか0未満の場合はエラー
        if isinstance(config["VIDEO"]["SOURCE"], str) and config["VIDEO"]["SOURCE"].endswith(".mp4"): # ソースが文字列で動画ファイルを参照している場合はpass
            pass
        else:
...
class VideoWriter:
    ...
    def __init__(self, width, height, fps, path=None):
        ...
        if path:
            self.base_path = os.path.join(self.MOTION_PATH, path) # 動体検知時の保存パス
        else:
            self.base_path = datetime.now().strftime("%Y-%m-%d-%H-%M-%S") # デフォルトの保存パス
